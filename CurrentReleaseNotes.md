...
- **Safer carrier cargo cache** - Ravencolonial server snapshots, CAPI snapshots, and journal cargo deltas now have clearer ownership. Full server snapshots replace the local manifest, CAPI cannot overwrite a newer non-empty server baseline without freshness evidence, and journal deltas keep the overlay view current. If a selected project-linked carrier has no local manifest yet, the overlay makes one guarded seed request and shows `sync` instead of a false negative deficit if cargo still cannot be loaded.
