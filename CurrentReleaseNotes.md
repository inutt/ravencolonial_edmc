...
- **Safer carrier cargo cache** - Ravencolonial server snapshots, CAPI snapshots, and journal cargo deltas now have clearer ownership. Full server snapshots replace the local manifest, CAPI cannot overwrite a newer non-empty server baseline without freshness evidence, and journal deltas keep the overlay view current.
